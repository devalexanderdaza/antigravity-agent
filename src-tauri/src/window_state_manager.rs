...
impl WindowState {
    ...
    pub fn is_valid(&self) -> bool {
        ...
        let position_valid = self.x > -1000.0 && self.x < 10000.0 
                          && self.y > -1000.0 && self.y < 10000.0;
        
        // 检查窗口大小是否合理（400x400 到 4000x3000）
        let size_valid = self.width >= 400.0 && self.width <= 4000.0
                      && self.height >= 400.0 && self.height <= 3000.0;
...
pub async fn save_window_state(state: WindowState) -> Result<(), String> {
    // 验证窗口状态是否有效，拒绝保存异常值
    if !state.is_valid() {
        println!("⚠️ 检测到无效的窗口状态，跳过保存: 位置({:.1}, {:.1}), 大小({:.1}x{:.1})",
...
pub async fn load_window_state() -> Result<WindowState, String> {
    ...
    if state_file.exists() {
        ...
        // 验证加载的状态是否有效
        if !state.is_valid() {
            println!("⚠️ 加载的窗口状态无效（位置({:.1}, {:.1}), 大小({:.1}x{:.1})），使用默认状态",
                     state.x, state.y, state.width, state.height);
